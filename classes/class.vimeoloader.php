...
class VimeoLoader {
    ...
    function __construct(&$modx,$user='user8504353') {
    ...
    public function getAllVideos($vid='',$w=400,$h=275) {
        ...
        $output = $this->modx->getChunk('vl_vimeoFeedWrapperTpl',array('content' => $allvids));
		return $output;
        
    }
    
    public function getFeaturedVideo($vid='',$w=675,$h=380,$wrapper='vl_featuredVideoWrapperTpl') {	
    	...
    	$video = array();
    	$video['content'] = $this->modx->getChunk('vl_vimeoVideoIFrameTpl',$data);    
    	
    	if ($wrapper != '') {
    		if ($output = $this->modx->getChunk('vl_featuredVideoWrapperTpl',$video)) {
    			return $output;
    		} else {
